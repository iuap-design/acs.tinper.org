...
export const getExtImg = (ext) => {
    //所有图片
    let imgExts = ['jpg','webp', 'bmp', 'pcx', 'tif', 'gif', 'jpeg', 'tga', 'exif', 'fpx', 'svg', 'psd', 'cdr', 'pcd', 'dxf', 'ufo', 'eps', 'png', 'hdri', 'raw', 'wmf', 'flic', 'emf', 'ico'];
    //所有音频
    let audioExts = ['mp3','aac','wav','wma','cda','flac','m4a','mid','mka','mp2','mpa','mpc','ape','ofr','ogg','ra','wv','tta','ac3','dts'];
    //所有视频
    let videoExts = ['avi','asf','wmv','avs','flv','mkv','mov','3gp','mp4','mpg','mpeg','dat','ogm','vob','rm','rmvb','ts','tp','ifo','nsv'];
    //所有压缩
    let zipExts = ['rar','zip','arj','z','cab','lzh','ace','tar','gz','uue','bz2','jar','iso','7z'];
 ...
 */
export const getFileNames = (name, ext) => {
    ext = ext.split('.')[1];
    return <span className = 'name-space' title={name+ext}>
                {/* <img src = {getExtImg(ext)}/>  */}
                <span> { name } </span> 
            </span>
}
